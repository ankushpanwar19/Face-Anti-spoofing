...
def get_frm_list(mode, protocol, img_path, sel_every, sel_these_many, disc_frms, flag, fldnm, split):
    ...
    if (mode == 'train') and flag != 1 and (disc_frms > 0):
        ...
        frm_ids = set(random.sample(range(len(frm_ids)), sel_these_many))
        frm_ids = list(frm_ids)
    elif (mode == 'val') and (sel_every > 1) and (sel_these_many == 0) and (len(frm_ids) > 2 * sel_every):
        frm_ids = frm_ids[::sel_every]
    ...
    with open(fname) as f:
        for line in f:
            ...
            if flag == 1:
                ...
                print('error found in get_examples_labels_replaymobile.py --> read_proto_file(): flag value is not correct!!!')
                sys.exit()
            ...
            for i in flist:
                ...
                if flag == 1:
                    ...
                    if num_cls == 3:
                        ...
                        print('error found in get_examples_labels_replayattack.py --> read_proto_file(): num_cls value is not correct!!!')
                        sys.exit()
...
def get_part_labels(mode, protocol, proto_path, img_path, sel_every, sel_these_many, disc_frms, split, net_type, small_trainset = False, datasetID = None, num_cls = 2):
    ...
    if mode == 'train':
        ...
        fname = join(proto_path, 'Dev.txt')
    elif mode == 'test':
        fname = join(proto_path, 'Test.txt')
    # print('>>>>> get_examples_labels_replayattack.py --> get_part_labels() --> proto-fname: {} '.format(fname))
...
def get_examples_labels(datset_path, mode, protocol, split, sel_every, sel_thesemany, img_path, net_type, small_trainset = False, datasetID = None, num_cls = 2):
    ...
    if mode == 'train':
        print('+++ during {} mode [sel_these_many] is set by uitls.py --> get_examples_labels() +++'.format(mode))
    elif mode == 'test' or mode == 'val':
        print('+++ during {} mode [sel_these_many] is set by the configs.yaml --> sel_these_many={} +++'.format(mode, sel_these_many))
    else:
        pass
